...
#include <iostream>
#include <string>
#define CLIENTWOTLKLIB_DLL "ClientWotlkLib.dll"
...
int main(int argc, char** argv)
{
    ...
    std::filesystem::path libInGamePath = std::filesystem::path(exePath).parent_path() / CLIENTWOTLKLIB_DLL;
    std::filesystem::path libInAppPath = std::filesystem::absolute(argv[0]).parent_path() / CLIENTWOTLKLIB_DLL;
    ...
    if (std::filesystem::is_regular_file(libInGamePath)) {
        ...
        message(MB_ICONWARNING,
            "Patch succesfully applied\n"
            "But look like `" CLIENTWOTLKLIB_DLL "` is missed\n"
